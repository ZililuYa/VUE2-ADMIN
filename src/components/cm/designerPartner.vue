<template>
  <div class="com sys-user">
    <div class="ani">
      ...
      <Select v-model="parameter.partnerStatus" @on-change="getData()">
...
<script>
  ...
  export default {
    data() {
      return {
        judge: {
          recommend: this.$judge('cm.designerPartner.recommend'),
          del: this.$judge('cm.designerPartner.del'),
          edit: this.$judge('cm.designerPartner.edit'),
          view: this.$judge('cm.designerPartner.view'),
          trial: this.$judge('cm.designerPartner.trial'),
          enable: this.$judge('cm.designerPartner.enable'),
          disable: this.$judge('cm.designerPartner.disable')
        },
        data: [],
        total: 0,
        certificateImg: [],
        ...
        columns: [
          ...
          },
          {
            ...
            key: 'createTime',
            render: (h, params) => {
              if (params.row.partnerStatus === 1) {
                return this.$lang.currency.pAudit
              } else if (params.row.partnerStatus === 2) {
                return this.$lang.currency.auditP
              } else if (params.row.partnerStatus === 3) {
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              if (this.judge.trial && (params.row.partnerStatus === 1)) {
                arr.push(h('Button', {
                  props: {
                    size: 'small'
                  },
              ...
              if (this.judge.recommend && params.row.recommand === 1 && params.row.partnerStatus === 2) {
                arr.push(h('Button', {
                  props: {
                    size: 'small'
                  },
              ...
              if (this.judge.recommend && params.row.recommand === 2 && params.row.partnerStatus === 2) {
                arr.push(h('Button', {
                  props: {
                    size: 'small'
                  },
              ...
              if (this.judge.enable && (params.row.auditStatus === 4 || params.row.partnerStatus === 4)) {
                arr.push(h('Button', {
                  props: {
                    size: 'small'
                  },
              ...
              if (this.judge.disable && (params.row.auditStatus === 2 || params.row.partnerStatus === 2)) {
                arr.push(h('Button', {
                  props: {
                    size: 'small'
                  },
              ...
              if (this.judge.del && (params.row.partnerStatus === 2 || params.row.partnerStatus === 3)) {
                arr.push(h('Button', {
                  props: {
                    type: 'error',
                    size: 'small'
    ...
    props: {},
    mounted() {
      this.SYSTEM_SETPOSITION([{title: this.$lang.currency.home, path: '/home'}, {title: this.$lang.currency.oMan}, {title: '设计师合作伙伴管理'}])
      this.getData()
      this.getClass()
    },
    filters: {
    ...
    methods: {
      ...
      auditItem(id, num) {
        ...
        let info = num === 2 ? '' : '不'
        if (num !== 2) {
          ajax(api.partnerAudit, 'POST', {id: id, partnerStatus: num, reason: this.value}, callback)
          return
        }
        this.$Modal.confirm({
          title: this.$lang.currency.prompt,
          content: '确定' + info + '通过吗？',
          onOk: () => {
            ajax(api.partnerAudit, 'POST', {id: id, partnerStatus: num}, callback)
          },
          onCancel: () => {
          }
        })
