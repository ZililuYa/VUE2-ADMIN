...
<script>
  ...
  export default {
    ...
    data() {
      return {
        judge: {
          add: this.$judge('cm.help.add'),
          del: this.$judge('cm.help.del'),
          view: this.$judge('cm.help.view'),
          edit: this.$judge('cm.help.edit')
        },
        pro: 0,
        canCrop: false,
    ...
    mounted() {
      this.getData()
      this.init()
      this.SYSTEM_SETPOSITION([{title: this.$lang.currency.home, path: '/home'}, {title: this.$lang.currency.oMan}, {title: '帮助文档管理'}])
    },
    methods: {
          ...
          if (valid && this.modalItem.content) {
            this.$Modal.confirm({
              ...
              onOk: () => {
                ...
                ajax(this.edit ? api.helpUpdate : api.helpAdd, 'POST', this.modalItem, callback, this.clearLoading)
              },
              onCancel: () => {
              }
            })
...
//        }
//        ajax(api.helpType, 'GET', {}, callback)
      },
      changePage,
      changePageSize,
      viewItem(item) {
      ...
      removeItem(id) {
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            ajax(api.helpDel, 'POST', {articleId: id}, callback)
          },
          onCancel: () => {
          }
        })
