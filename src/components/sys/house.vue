<template>
  <div class="com sys-user">
    <div class="ani">
      ...
      <Select v-model="parameter.modelState" @on-change="getData()">
    ...
    <Modal
      ...
      <div class="modal_def">
        <div class="img">
          <img v-if="modalItem.picUri" v-lazy="modalItem.picUri" alt="">
          <span v-if="!modalItem.picUri">没有预览图</span>
          <br>
        </div>
        <div class="li">
...
<script>
  ...
  export default {
    data() {
      return {
        judge: {
          view: this.$judge('sys.house.view'),
          trial: this.$judge('sys.house.trial')
        },
        parent: '',
        data: [],
        total: 0,
        ...
        columns: [
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              if (this.judge.trial && this.parameter.modelState === '1') {
                arr.push(h('Button', {
                  props: {
                    size: 'small'
                  },
    ...
    props: {},
    mounted() {
      this.SYSTEM_SETPOSITION([{title: this.$lang.currency.home, path: '/home'}, {title: this.$lang.currency.oMan}, {title: '户型管理'}])
      this.getData()
    },
    methods: {
      ...
      auditItem(id, num) {
        ...
        let info = num === 2 ? '' : '不'
        if (num !== 2) {
          ajax(api.houseAudit, 'POST', {id: id, state: num, reason: this.value}, callback)
          return
        }
        this.$Modal.confirm({
          ...
          content: '确定' + info + '通过吗？',
          onOk: () => {
            ajax(api.houseAudit, 'POST', {id: id, state: num}, callback)
          },
          onCancel: () => {
          }
        })
